...
independently in Section~3 from the market geodesic equation alone, and
it recovers exactly the mathematical form that governs relativistic
acceleration in physics. The convergence of two independent derivations
--- one from market microstructure, one from spacetime geometry ---
arriving at the same functional form is the central empirical anchor for
the broader hypothesis developed in this Preface.
...
zones and simultaneously appears as the Nash equilibrium of the traders'
liquidity-cost optimisation game. The Lyapunov time
...
Entropy is developed as a market parameter via the Bekenstein-type bound
$S_{\mathcal{M}} \leq 2\pi A\tau_L/c$, bridging thermodynamic,
information-theoretic, and dynamical-systems descriptions of an impulse.
...
The threshold $v/c = 1/\varphi \approx 0.618$ appears on three
independent levels:

\begin{center}
\begin{tabular}{lll}
\toprule
Level & Appearance & Origin \\
\midrule
Geometric & Inflection point of $\gamma(v)$ & $d^3\gamma/dv^3 = 0 \Rightarrow v=c/\sqrt{3}\approx 0.577c$ \\
Algorithmic & Golden-section search & Optimal partition for unimodal $F(c)$ \\
Game-theoretic & Nash equilibrium & $\gamma$ vs.\ $\gamma^3$ cost game, leading order \\
\bottomrule
\end{tabular}
\end{center}

The three dynamical zones:
...
\begin{equation}
    \boxed{S_{\mathcal{M}} \leq \frac{2\pi A\tau_L}{c}.}
\end{equation}
...
The non-zero Christoffel symbols for $g_{\mu\nu}(x)=\text{diag}(-c(x)^2,1)$:
\begin{align}
    \Gamma^0_{00} &= \frac{\partial_0 c}{c}, &
    \Gamma^0_{11} &= -\frac{\partial_1 c}{c}, \\
    \Gamma^1_{00} &= c\,\partial_1 c, &
    \Gamma^1_{01} &= \Gamma^1_{10} = \frac{\partial_0 c}{c}.
\end{align}

The geodesic equation:
\begin{equation}
    \ddot{(\ln P)} + c\,\partial_1 c\,\dot{(\ln V)}^2
    + \frac{2\partial_0 c}{c}\,\dot{(\ln V)}\dot{(\ln P)}
    - \frac{\partial_1 c}{c}\,\dot{(\ln P)}^2 = 0.
\end{equation}
When $c$ is constant every derivative term vanishes, recovering the flat-
space result $\ddot{(\ln P)}=0$ exactly. Curved space-time is the strict
generalisation.
