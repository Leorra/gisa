...
namespace gisa {
	...
	public:
		...
		// ---- Thermodynamic extension: real-market c_eff layer (thesis §12.3) ----
		// Configure the fibre parameters (G, C) and their sensitivities.
		// When enabled, c_eff = c0 · exp(-λ_G·G/G0 − λ_C·C/C0) replaces the
		// raw c supplied to update().  Disabled by default (lambda_G = 0).
		struct ThermodynamicFibre {
			T lambda_G = 0.0;   // sensitivity to government intervention
			T lambda_C = 0.0;   // sensitivity to institutional friction
			T G = 0.0;   // current normalised government spending G/G0
			T C = 0.0;   // current normalised corruption index C/C0
			bool enabled() const noexcept { return lambda_G > 0.0 || lambda_C > 0.0; }
			// Returns the deformation factor f ∈ (0,1] applied to c0.
			T deformation() const noexcept {
				return std::exp(-lambda_G * G - lambda_C * C);
			}
			// Returns c_eff given the raw baseline c0.
			T c_eff(T c0) const noexcept { return c0 * deformation(); }
		};

		void set_thermodynamic_fibre(ThermodynamicFibre f) noexcept {
			thermo_fibre_ = f;
		}
		void update_fibre_G(T G_norm) noexcept { thermo_fibre_.G = G_norm; }
		void update_fibre_C(T C_norm) noexcept { thermo_fibre_.C = C_norm; }
		const ThermodynamicFibre& fibre() const noexcept { return thermo_fibre_; }

		// Freeze-point check: returns true when |v| >= c_eff (thesis §12.9).
		// Caller provides current market velocity v and raw baseline c0.
		bool at_freeze_point(T v, T c0) const noexcept {
			if (!thermo_fibre_.enabled()) return false;
			return std::abs(v) >= thermo_fibre_.c_eff(c0);
		}

		// True during the quiet period after a c-collapse BREAKING trigger.
		// GeodesicTracker uses this to escalate signal without re-emitting BREAKING.
		bool in_c_collapse_cooldown() const noexcept {
			return c_collapse_cooldown_ > 0;
		}
		...
		void update(T lnV, T lnP, T c, T timestamp = 0.0) {
			...
			// Apply thermodynamic fibre deformation (thesis §12.3) if configured.
			// c_raw is the market-supplied baseline; c_effective may be further
			// deformed by the SG rolling window below.
			T c_effective = thermo_fibre_.enabled() ? thermo_fibre_.c_eff(c) : c;
			...
			// Decrement cooldown after detect_breaking so the quiet period applies
			// starting from the bar *after* the triggering collapse.
			if (c_collapse_cooldown_ > 0) --c_collapse_cooldown_;
		...
		bool detect_breaking(T prev_c = -1.0) const {
			...
			// Gated by cooldown to suppress oscillation on monotone-declining c
			// sequences where every bar would otherwise re-trigger BREAKING.
			if (c_collapse_thr_ > 0.0 && use_prev_c > 1e-12 && !history_.empty()
				&& c_collapse_cooldown_ == 0)
			{
				const T dc_rel = (current_.c - use_prev_c) / use_prev_c;
				if (dc_rel < -c_collapse_thr_) {
					c_collapse_cooldown_ = kCCollapseQuietBars;
					return true;
				}
			}
		...
		T average_curvature(size_t w = 0) const {
			...
			// Clamp each |R| contribution at breaking_R_thr_ before accumulating.
			// A single phase-explosion spike (|R| > 1e6) would otherwise dominate
			// the running average and produce a misleading summary statistic.
			const T clamp = breaking_R_thr_;
			T s = 0.0;
			for (size_t i = history_.size() - n; i < history_.size(); ++i) {
				const T r = std::get<3>(history_[i]);
				s += std::copysign(std::min(std::abs(r), clamp), r);
			}
	...
	private:
		...
		T compute_xi_curved(T lnP_obs, T v, T /*c*/,
			const ChristoffelSymbols& cs) const noexcept
		{
			if (!entry_set_) return 0.0;
			// proper_tau_ = Σ |dlnV|/γ accumulated since set_entry() —
			// the invariant proper-time elapsed in log-volume coordinates.
			const T tau = proper_tau_;
			const T u0 = 1.0;   // d(lnV)/dτ = 1 (parametrisation choice)
			const T u1 = v;     // d(lnP)/dτ = v
			// Geodesic acceleration for x¹ = lnP (thesis §5.3, eq. geodesic_curved):
			//   ẍ¹ = -Γ¹₀₀ (u⁰)²
			// Γ¹₀₁ = 0 identically — the cross-term does not appear.
			const T a1 = -(cs.G1_00 * u0 * u0);
		...
		T      min_xi_, max_xi_;
		ThermodynamicFibre thermo_fibre_ {};

		// c-collapse cooldown: once a c-collapse BREAKING is emitted, suppress
		// re-triggering for this many bars to prevent oscillation on monotone
		// declining c series. Only the c-collapse branch (condition 2) is gated;
		// velocity-singularity and xi/ds2 paths remain unconditional.
		static constexpr size_t kCCollapseQuietBars = 3;
		mutable size_t c_collapse_cooldown_ = 0;  // bars remaining in quiet period
	};
