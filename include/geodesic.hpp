...
namespace gisa {
	...
	public:
		...
		ExitSignal evaluate(const ManifoldPoint& pt) {
			return evaluate_impl(pt, false);
		}

		// Overload accepting the tracker so cooldown state can escalate the signal.
		ExitSignal evaluate(const ManifoldPoint& pt, const ManifoldTracker& tracker) {
			return evaluate_impl(pt, tracker.in_c_collapse_cooldown());
		}
	...
	private:
		...
		ExitSignal evaluate_impl(const ManifoldPoint& pt, bool in_collapse) {
			if (!initialized_) {
			...
			// Large absolute curvature spike at any v/c — structural instability.
			// Threshold 500 suppresses SG noise (<100) while catching genuine
			// phase-explosion R spikes independent of velocity.
			if (std::abs(pt.R) > 5.0e2)                return ExitSignal::SELL;
			...
			// ---- Prepare exit ----
			// Escalate to PREPARE_EXIT during c-collapse cooldown period: the
			// manifold is in a confirmed liquidity-drain trajectory even though
			// BREAKING is suppressed for this bar.
			if (in_collapse)                            return ExitSignal::PREPARE_EXIT;
